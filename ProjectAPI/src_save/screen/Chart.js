...
class ChartScreen extends React.Component {
   constructor(props) {
      ...
      this.high_level_btn=this.high_level_btn.bind(this);
      this.low_level_btn=this.low_level_btn.bind(this);
      this.temperature_btn=this.temperature_btn.bind(this);
  ...
  get_token_init(){
   ...
   high_level_btn()
   {
      this.update_data(this.state.high_level_id,'High level');
   }
   low_level_btn()
   {
      this.update_data(this.state.low_level_id,'Low level');
   }
   temperature_btn()
   {
      this.update_data(this.state.low_level_id,'Temperature');
   }
   update_btn()
   {
   ...
   render_sensor_btn(item,index) {
    ...
    <View style={{width:'50%',justifyContent: "center",alignItems: 'center',paddingBottom:10}} key={index}>
      <Button title={item.name} size="sm" bgColor="black" textColor="white" onPress={() => this.update_data(index)}/>
    </View>
    );
  }
	render() {
